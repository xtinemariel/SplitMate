...
function validateExpenseDate(
  ...
  if (parsed < minDate && value !== allowedExistingDate) {
...
export function ExpenseForm({
  ...
  const minDateValue =
    existingExpenseDate && existingExpenseDate < oneYearAgoInputValue()
      ? existingExpenseDate
      : oneYearAgoInputValue();

  const [values, setValues] = useState<ExpenseFormValues>(() => ({
    amount: initialValues?.amount ?? "",
    description: initialValues?.description ?? "",
    paidByGroupMemberId: initialValues?.paidByGroupMemberId ?? defaultPayer,
    expenseDate: initialValues?.expenseDate ?? defaultExpenseDate,
    selectedParticipants:
      initialValues?.selectedParticipants ?? members.map((member) => member.id),
  }));
  ...
  function handleDateChange(value: string) {
    ...
    if (touched.expenseDate) {
      setErrors((current) => ({
        ...current,
        expenseDate: validateExpenseDate(value, existingExpenseDate) ?? undefined,
      }));
    }
  }
  ...
  function handleDateBlur() {
    ...
    setErrors((current) => ({
      ...current,
      expenseDate:
        validateExpenseDate(values.expenseDate, existingExpenseDate) ?? undefined,
    }));
  }
